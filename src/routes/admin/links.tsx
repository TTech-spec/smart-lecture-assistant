...
import {
  ...
  fetchLinksFromSupabase, saveLinks,
  type AttendanceLink,
    ...
    >
      ...
      >
        ...
        <p className="mt-3 text-xs text-muted-foreground">
          Students can scan this QR code to open the attendance link directly.
        </p>
...
// ── Main page ─────────────────────────────────────────────────────────────────
function LinksPage() {
  const { links, records, settings } = useStore();
  const [showForm, setShowForm] = useState(false);
  const [qrLink, setQrLink] = useState<AttendanceLink | null>(null);
  ...
  const courses = useMemo(
    () => Array.from(new Set([...(settings.courses || []), ...links.map((l) => l.courseCode)])).sort(),
    [settings.courses, links],
  );

  const activeCount = links.filter((l) => isLinkValid(l)).length;
  ...
  return (
    <main className="mx-auto max-w-5xl px-4 pb-16 pt-6 sm:px-6 sm:pt-8">
      ...
      >
        {sorted.length === 0 ? (
          <div className="flex flex-col items-center gap-3 px-4 py-16 text-center">
            ...
            <p className="text-base font-medium">No attendance links yet</p>
            <p className="text-sm text-muted-foreground max-w-xs">
              Click "New link" to generate a shareable URL. Students open it to mark attendance.
            </p>
      ...
      <AnimatePresence>
        {qrLink && (
          <QrModal
            url={buildAttendUrl(qrLink.token)}
            title={qrLink.title}
